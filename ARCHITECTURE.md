...
| `physics_window.py` | 575 | Physics settings panel: slider groups, multi-load mode, parameter-lock alt-click hooks |
...
| File | Lines | Description |
|------|-------|-------------|
| `sim.py` | 914 | GPU particle simulation: buffers, compute dispatch, physics→uniform mapping, sweeps, multi-load SSBO packing, rules (**user-owned**) |
...
| `multi_load_service.py` | 150 | Mix up to 64 configs simultaneously; progression/cohort assignment; SSBO-dirty flag |
...
| `ui_state.py` | 103 | **Aggregate frame snapshot** nesting Sim/Camera/Recording/Preferences/MultiLoad + ~60 one-shot flags |
| `camera_state.py` | 29 | 2D + 3D camera state + OptiX/path-tracer enable and timing readouts |
| `multi_load_state.py` | 24 | Multi-load toggle + progression/assignment settings |
...
- Config save/load and multi-load push rules to the stack
...
### Multi-Load
Mix up to 64 saved configs across particle cohorts. Python state lives in `MultiLoadService`; the actual per-config SSBO is packed in `sim.py` (`_write_multi_load_ssbo`, user-owned). Mutually exclusive with parameter sweeps and parameter locks.
