...
### Step 6 — Merge the two OptiX renderers
...
### Step 8 — Viewer window + overlay pass; begin 3D-only
...
- **Files:** new `viewer/`; `main.py` (`_render_camera_view` → Viewer), `camera.py` (`cam_brush` display path, `cam_brush_target` FBO disentangle), `ui/core.py` (Viewer in render dispatch, exempt from hide-all), `shaders/frame_assembly.frag` (strip overlay markup), overlay shaders.
- **Verify:** Viewer window always visible, survives "hide windows"; reticle/sweep/draw-cursor overlays render over the 3D output; recording still captures the clean (markup-free) frame; drawing still works (or is a documented follow-up).

### Step 9 — Modularize Config Clipboard
...
- **Files:** new `config_clipboard/` + `state/config_clipboard_state.py`; move clipboard state from `ui/core.py:109`–115; rename/relocate `ui/history_window.py` → clipboard window + a separate tooltip module; `command_handler.py:515`–635 handlers; `services/field_handler.py` snapshot cap.
