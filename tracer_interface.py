...
class TracerInterface:
    ...
    def render_blocking(self, entity_buffer: moderngl.Buffer, entity_count: int,
                        view_proj: np.ndarray, width: int = 512, height: int = 512):
        """Splat entities, path-trace to completion, tonemap to display texture.

        This is a BLOCKING call -- the entire render completes before returning.
        """
        ...
        # Build render params from current slider state
        medium, sun, sky, render = self._build_params()

        # Path-trace to completion (blocking)
        self._renderer.render_to_completion(
            view_proj, self._target_tex,
            medium, sun, sky, render
        )

        # CPU-side tonemap into display texture
        self._tonemap_to_display()

        self._last_spp = self.num_samples
    ...
    def display_texture(self) -> moderngl.Texture | None:
        ...
        return self._display_tex if self._render_complete else None
