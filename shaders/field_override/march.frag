...
#define SMALL_BUMP HIT_DISTANCE*10
#define MAX_DISTANCE 1200

#define FOCAL_LENGTH .75
...
#define PHASE generic03.y//frame_count/100000.+.01//generic03.y//(frame_count/960./3.)
MR map(vec3 p){//pR(p.xz,frame_count/10500.);
//pR(p.xz,PHASE);
//p.x-=fract(PHASE)*14;
p.x=-abs(p.x);
float ground = p.y+1.1;
ground -= -40*fbm(vec2(generic03.w*40),.5)+40*fbm(generic03.w*40+p.xz/160.,.5);
float dts = min(ground,tower(p));

//dts = ornament(p-vec3(0,10,0));
vec4 mat = dts==ground?vec4(0,.2,.2,1):vec4(1,.061,.031,1);
MR facade = tower_facade(p);
dts = min(dts,facade.dts);
if(dts==facade.dts){mat.x=facade.mat.x;}
mat.x = dts > 2.?3.:mat.x;
return MR(dts,mat);
}
...
vec3 cam_curve(float t){
    return vec3(0);//vec3(-t/700/4,pow(t/700/4,1.5),0);
}
vec3 forward;
vec3 right;
vec3 up;
Ray getCam(vec2 uv){
    ...
    vec3 ray_ori = camera_pos;
    
    //scene modification
    ray_ori += cam_curve(frame_count);

    return Ray (ray_ori,dir,0,vec3(0));
}
...
#define SUN_DIR (normalize(vec3(sin(PHASE*3.14),1.1,cos(PHASE*3.14)))).zxy//(normalize(vec3(.1,1,1)))
out vec4 fragColor;
void main(void)
{
    ...
    float light = max(0.,dot(SUN_DIR,cam_ray.norm));
    if(light>0){light *= occlude_march(cam_ray,SUN_DIR);}
    //vec2 field0 = vec2(cam_ray.extent/30.);
    vec3 N = cam_ray.norm;
    vec3 D = normalize(vec3(-1,.1,0));//down vector in worldspace
    //pR(D.xz,PHASE);
    ...
    field0 = field0*(exp(generic03.z*-cam_ray.extent));
    ...
    fragColor = vec4(-field0,light,hit.mat.x);//vec4(field0,-field0);
